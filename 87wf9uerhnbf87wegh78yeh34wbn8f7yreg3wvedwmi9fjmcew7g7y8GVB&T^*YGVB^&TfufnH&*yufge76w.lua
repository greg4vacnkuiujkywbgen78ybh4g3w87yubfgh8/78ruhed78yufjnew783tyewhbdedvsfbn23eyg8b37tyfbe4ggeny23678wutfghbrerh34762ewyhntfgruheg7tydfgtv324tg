...
local btn7 = Instance.new("ImageButton") setupIcon(btn7, aaframe, "rbxassetid://71833091806293") --111787479476552
btn7.Position = UDim2.new(0.5, 0.3, 0.5, 3)

local btn2 = Instance.new("ImageButton") setupIcon(btn2, legitframe, "rbxassetid://109522202954909") --118424702230055
btn2.Position = UDim2.new(0.5, 0.3, 0.5, 3)

local btn3 = Instance.new("ImageButton") setupIcon(btn3, visualframe, "rbxassetid://134337510002621") --76116815149256
btn3.Position = UDim2.new(0.5, 0.3, 0.5, 3)

local btn4 = Instance.new("ImageButton") setupIcon(btn4, settingsframe, "rbxassetid://96057809454753") --81242647298145
btn4.Position = UDim2.new(0.5, 0, 0.5, 3)

local btn5 = Instance.new("ImageButton") setupIcon(btn5, skinframe, "rbxassetid://129738887148306") --137249269545091
btn5.Position = UDim2.new(0.5, 4, 0.5, 2)

local btn6 = Instance.new("ImageButton") setupIcon(btn6, userframe, "rbxassetid://115027436437851") --112858167638205
btn6.Position = UDim2.new(0.5, 3, 0.5, 4)

local btn8 = Instance.new("ImageButton") setupIcon(btn8, extraframe, "rbxassetid://88959969303399") --121244660498213
...
local SettingsServer = Library.Tabs.Settings:AddSection("Server", "Left")
...
local JobIdInput = SettingsServer:AddInputBox({
    Text = "Job ID",
    Default = "",
...
SettingsServer:AddButton({
    Text = "Join Job ID",
    Callback = function()
        local jobId = JobIdInput.GetText()
        if jobId and jobId ~= "" then
            Library:Notify("Joining server...")
            game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, jobId, LocalPlayer)
        else
            Library:Notify("Please enter a Job ID")
        end
    end
})
...
SettingsServer:AddButton({
    ...
    Callback = function()
        ...
        if #servers > 0 then
            ...
            Library:Notify("No available servers found")
        end
    end
})

local UserMovement = Library.Tabs.User:AddSection("Movement", "Right")
...
local function startFlying()
    ...
    flyConnection = game:GetService("RunService").Heartbeat:Connect(function()
        ...
        if keys.W then moveDirection = moveDirection + cameraCFrame.LookVector end
        if keys.S then moveDirection = moveDirection - cameraCFrame.LookVector end
        if keys.A then moveDirection = moveDirection - cameraCFrame.RightVector end
        if keys.D then moveDirection = moveDirection + cameraCFrame.RightVector end
...
local EmoteSettings = {
    ...
    IsToggleOn = false,
    PreAddedEmotes = {
        ["Floating"] = "88425531063616",
        ["Catterpillar"] = "120379657846115",
        ["Tall Character"] = "79216795769647",
        ["Biblically Accurate Angel"] = "70544785852547",
        ["The Worm"] = "135982756481001"
    }
}
...
local function StopTrack()
    if EmoteSettings.CurrentTrack then
        EmoteSettings.CurrentTrack:Stop(0.1)
        EmoteSettings.CurrentTrack = nil
    end
    if EmoteSettings.EmoteLoop then
        EmoteSettings.EmoteLoop:Disconnect()
        EmoteSettings.EmoteLoop = nil
    end
end
...
local function LoadTrack(id)
    ...
    if success and result and #result > 0 and result[1]:IsA("Animation") then
    ...
    EmoteSettings.CurrentTrack = newTrack
    
    EmoteSettings.EmoteLoop = RunService.RenderStepped:Connect(function()
        if not EmoteSettings.CurrentTrack or not EmoteSettings.CurrentTrack.IsPlaying then
            StopTrack()
        end
    end)
end
...
UserMovement:AddToggle({
    ...
    Default = false,
    Callback = function(val)
        EmoteSettings.IsToggleOn = val
        if val then
            local id = extractIdFromInput(EmoteSettings.EmoteId)
            if id then
                LoadTrack(id)
            else
                StopTrack()
...
local EmoteInputBox = UserMovement:AddInputBox({
    Text = "Emote ID",
    Default = "",
    Placeholder = "Enter ID",
    Callback = function(val)
        EmoteSettings.EmoteId = val
    end
})

UserMovement:AddDropdown({
    Text = "Pre-Added Emotes",
    Options = {"Floating", "Catterpillar", "Tall Character", "Biblically Accurate Angel", "The Worm"},
    Callback = function(val)
        local id = EmoteSettings.PreAddedEmotes[val]
        if id then
            EmoteSettings.EmoteId = id
            
            if EmoteInputBox and EmoteInputBox.SetText then
                EmoteInputBox.SetText(id)
            end

            if EmoteSettings.IsToggleOn then
                LoadTrack(id)
            end
        end
    end
...
UserMovement:AddSlider({
    Text = "Speed",
    Min = 0, Max = 5, Default = 1, Decimals = 2,
    Callback = function(val)
        EmoteSettings.Speed = val
        if EmoteSettings.CurrentTrack then 
            EmoteSettings.CurrentTrack:AdjustSpeed(val) 
        end
    end
})
...
local ExtraMain = Library.Tabs.Extra:AddSection("Main", "Right")
...
ExtraMain:AddDropdown({
    Text = "Animation Style",
    Options = {"Quint", "Quad", "Sine", "Back", "Bounce", "Elastic", "Linear", "Exponential", "Circular"},
    Default = "Quint",
    Callback = function(val)
        Library.AnimationEase = Enum.EasingStyle[val]
    end
})
...
ExtraMain:AddButton({
    ...
    Callback = function()
        Library:Notify("The quick brown fox jumped over the lazy dog")
    end
})
...
ExtraMain:AddSlider({
    ...
    Suffix = "s",
    Callback = function(val)
        Library.NotifySettings.Duration = val
    end
})

ExtraMain:AddDropdown({
    Text = "Notify Position",
    Options = {"Top", "Center", "Bottom"},
    Default = "Top",
    Callback = function(val)
        Library.NotifySettings.Anchor = val
        Library:UpdateNotifyPos()
    end
})

ExtraMain:AddSlider({
    Text = "Notify X Offset",
    Min = -500,
    Max = 500,
    Default = 0,
    Callback = function(val)
        Library.NotifySettings.OffsetX = val
        Library:UpdateNotifyPos()
    end
})

ExtraMain:AddSlider({
    Text = "Notify Y Offset",
    Min = 0,
    Max = 500,
    Default = 35,
    Callback = function(val)
        Library.NotifySettings.OffsetY = val
        Library:UpdateNotifyPos()
    end
})
...
ExtraMain:AddToggle({
    ...
    Default = true,
    Callback = function(v)
        if Library.WatermarkScreen then
            Library.WatermarkScreen.Enabled = v
        end
    end
})
...
ExtraMain:AddToggle({
    Text = "tooltip",
    Default = false,
    Tooltip = "This toggle has a tooltip",
    Callback = function(v)
...
local Presets = Library.Tabs.Extra:AddSection("Configs", "Left")

local ConfigInput 
local ImportInput 

local function GetConfigsWithAutoload()
    ...
    for _, name in ipairs(configs) do
        if name == autoload then
            table.insert(displayConfigs, name .. " (autoload)")
        else
            table.insert(displayConfigs, name)
        end
    end
...
local ConfigList = Presets:AddListBox({
    Values = GetConfigsWithAutoload(),
    Callback = function(val)
        -- Remove " (autoload)" suffix if present
        local cleanName = val:gsub(" %(autoload%)", "")
        if ConfigInput then ConfigInput.SetText(cleanName) end
    end
})

ConfigInput = Presets:AddInputBox({
    Text = "Config Name",
    Default = "",
...
Presets:AddButton({
    ...
    Callback = function()
        ...
        if name then 
            Library:Confirm({
                Title = "Load Config",
                Message = string.format('Are you sure you want to load "%s"?', name),
                ConfirmText = "Load",
                CancelText = "Cancel",
                Callback = function(confirmed)
                    if confirmed then
                        Library:LoadConfig(name)
                        Library:Notify(string.format('Loaded "%s" successfully!', name))
                    end
                end
            })
        else
            Library:Notify("Please select a config to load")
        end
    end
})
...
Presets:AddButton({
    ...
    Callback = function()
        ...
        if name and name ~= "" then
            Library:Confirm({
                Title = "Save Config",
                Message = string.format('Save/update config as "%s"?', name),
                ConfirmText = "Save",
                CancelText = "Cancel",
                Callback = function(confirmed)
                    if confirmed then
                        Library:SaveConfig(name)
                        ConfigList.Refresh(GetConfigsWithAutoload())
                        Library:Notify(string.format('Saved "%s" successfully!', name))
                    end
                end
            })
        else
...
Presets:AddButton({
    ...
    Callback = function()
        ...
        if name then
            Library:Confirm({
                Title = "Delete Config",
                Message = string.format('Are you sure you want to delete "%s"? This cannot be undone!', name),
                ConfirmText = "Delete",
                CancelText = "Cancel",
                Callback = function(confirmed)
                    if confirmed then
                        ...
                        Library:Notify(string.format('Deleted "%s" successfully!', name))
                    end
                end
            })
        else
            Library:Notify("Please select a config to delete")
        end
    end
})
...
Presets:AddDualButton({
    LeftText = "set autoload",
    RightText = "stop autoload",
    LeftCallback = function()
        local name = ConfigList.Get()
        if name then
            local cleanName = name:gsub(" %(autoload%)", "")
            Library:Confirm({
                Title = "set autoload",
                Message = string.format('Set "%s" to autoload on startup?', cleanName),
                ConfirmText = "Set",
                CancelText = "Cancel",
                Callback = function(confirmed)
                    if confirmed then
                        Library:SetAutoload(cleanName)
                        ConfigList.Refresh(GetConfigsWithAutoload())
                        Library:Notify(string.format('"%s" will now autoload!', cleanName))
                    end
                end
            })
        else
    ...
    RightCallback = function()
        ...
        if autoload then
            Library:Confirm({
                Title = "stop autoload",
                Message = string.format('Remove autoload from "%s"?', autoload),
                ConfirmText = "Remove",
                CancelText = "Cancel",
                Callback = function(confirmed)
                    if confirmed then
            ...
            Library:Notify("No config is set to autoload")
        end
    end
})
...
Presets:AddButton({
    Text = "Export",
    Callback = function()
        local str = Library:ExportConfig()
        if setclipboard then
            setclipboard(str)
...
ImportInput = Presets:AddInputBox({
    Text = "Import Code",
    Default = "",
    Flag = "ManualImportInput"
})

Presets:AddButton({
    Text = "Import from Box",
    Callback = function()
        local str = ImportInput.GetText()
        if str and str ~= "" then
            local success, result = pcall(function()
                return Library:ImportConfig(str)
            end)
            
            if success then
                Library:Notify("Config imported successfully!")
                ConfigList.Refresh(GetConfigsWithAutoload())
            else
                Library:Notify("Failed to import config: Invalid format")
            end
        else
            Library:Notify("Please enter a config code to import")
        end
    end
})
...
Library:Watermark({
    Name = "gamesense | " .. game.Players.LocalPlayer.Name .. " | beta",
    Enabled = true
})

Library:KeybindList()

local UserMain = Library.Tabs.User:AddSection("Player List", "Left")

UserMain:AddPlayerList({
    Height = 400
})
...
task.spawn(function()
    ...
    if autoload then
        task.wait(1)
        if isfile(Library.FolderName .. "/" .. autoload .. ".json") then
            Library:LoadConfig(autoload)
            Library:Notify(string.format('Auto-loaded "%s"', autoload))
        else
            Library:RemoveAutoload()
            Library:Notify("Autoload config not found, removed autoload")
        end
    end
end)
